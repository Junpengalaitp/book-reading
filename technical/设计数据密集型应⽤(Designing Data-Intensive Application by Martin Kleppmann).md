...
### MapReduce之后
## 第十一章 流处理
* 当输⼊是⼀个⽂件（⼀个字节序列），第⼀个处理步骤通常是将其解析为⼀系列记录。在流处理的上下
⽂中，记录通常被叫做事件（event），但它本质上是⼀样的：⼀个⼩的，⾃包含的，不可变的对象，
包含某个时间点发⽣的某件事情的细节。⼀个事件通常包含⼀个来⾃时钟的时间戳，以指明事件发⽣的
时间。
* 例如，发⽣的事件可能是⽤户采取的⾏动，例如查看⻚⾯或进⾏购买。它也可能来源于机器，例如对温
度传感器或CPU利⽤率的周期性测量。在“使⽤Unix⼯具进⾏批处理”的示例中，Web服务器⽇志的每⼀
⾏都是⼀个事件。
* 当输⼊是⼀个⽂件（⼀个字节序列）时，第⼀个处理步骤通常是将其解析为⼀系列记录。在流处理的上
下⽂中，记录通常被称为事件，但它本质上是⼀样的：⼀个⼩的，⾃包含的，不可变的对象，包含某个
时间点发⽣的事情的细节。⼀个事件通常包含⼀个时间戳，指示何时根据时钟来发⽣
* 例如，发⽣的事情可能是⽤户采取的⾏动，例如查看⻚⾯或进⾏购买。它也可能来源于机器，例如来⾃
温度传感器的周期性测量或者CPU利⽤率度量。在“使⽤Unix⼯具进⾏批处理”的示例中，Web服务器⽇
志的每⼀⾏都是⼀个事件。
* 事件可能被编码为⽂本字符串或JSON，或者以某种⼆进制形式编码，如第4章所述。这种编码允许你存
储⼀个事件，例如将其追加写⼊⼀个⽂件，将其插⼊关系型表，或将其写⼊⽂档数据库。它还允许你通
过⽹络将事件发送到其他节点以进⾏处理。
* 在批处理中，⽂件被写⼊⼀次，然后可能被多个作业读取。类似地，在流处理术语中，⼀个事件由⽣产
者（producer）（也称为发布者（publisher）或发送者（sender））⽣成⼀次，然后可能由多个消
费者（consumer）（订阅者（subscribers）或接收者（recipients））进⾏处理【3】。在⽂件系统
中，⽂件名标识⼀组相关记录；在流媒体系统中，相关的事件通常被聚合为⼀个主题（topic）或流 （stream）。
* 原则上，⽂件或数据库就⾜以连接⽣产者和消费者：⽣产者将其⽣成的每个事件写⼊数据存储，且每
个消费者定期轮询数据存储，检查⾃上次运⾏以来新出现的事件。这实际上正是批处理在每天结束时处
理当天数据时所做的事情。

### 消息系统
* 向消费者通知新事件的常⽤⽅式是使⽤消息传递系统（messaging system）：⽣产者发送包含事件的
消息，然后将消息推送给消费者。我们之前在“消息传递中的数据流”中介绍了这些系统，但现在我们将
详细介绍这些系统。
* 在这个发布/订阅模式中，不同的系统采取各种各样的⽅法，并没有针对所有⽬的的通⽤答案。为了区
分这些系统，问⼀下这两个问题会特别有帮助
  * 如果⽣产者发送消息的速度⽐消费者能够处理的速度快会发⽣什么？⼀般来说，有三种选择：系统
可以丢掉消息，将消息放⼊缓冲队列，或使⽤背压（backpressure）（也称为流量控制（flow
control）；即阻塞⽣产者，以免其发送更多的消息）。例如Unix管道和TCP使⽤背压：它们有⼀
个固定⼤⼩的⼩缓冲区，如果填满，发送者会被阻塞，直到接收者从缓冲区中取出数据（参⻅“⽹
络拥塞和排队”）。
  * 如果消息被缓存在队列中，那么理解队列增⻓会发⽣什么是很重要的。当队列装不进内存时系统会
崩溃吗？还是将消息写⼊磁盘？如果是这样，磁盘访问⼜会如何影响消息传递系统的性能。
  *  如果节点崩溃或暂时脱机，会发⽣什么情况？ —— 是否会有消息丢失？与数据库⼀样，持久性可
能需要写⼊磁盘和/或复制的某种组合（参阅“复制和持久性”），这是有代价的。如果你能接受有
时消息会丢失，则可能在同⼀硬件上获得更⾼的吞吐量和更低的延迟。

#### 直接从⽣产者传递给消费者
* UDP组播⼴泛应⽤于⾦融⾏业，例如股票市场，其中低时延⾮常重要【8】。虽然UDP本身是不可
靠的，但应⽤层的协议可以恢复丢失的数据包（⽣产者必须记住它发送的数据包，以便能按需重新
发送数据包）。
* ⽆代理的消息库，如ZeroMQ和nanomsg采取类似的⽅法，通过TCP或IP多播实现发布/订阅
消息传递。
* StatsD和Brubeck使⽤不可靠的UDP消息传递来收集⽹络中所有机器的指标并对其
进⾏监控。 （在StatsD协议中，只有接收到所有消息，才认为计数器指标是正确的；使⽤UDP将
使得指标处在⼀种最佳近似状态。
* 如果消费者在⽹络上公开了服务，⽣产者可以直接发送HTTP或RPC请求（参阅“通过服务进⾏数据
流：REST和RPC”）将消息推送给使⽤者。这就是webhooks背后的想法【12】，⼀种服务的回调
URL被注册到另⼀个服务中，并且每当事件发⽣时都会向该URL发出请求。
* 尽管这些直接消息传递系统在设计它们的环境中运⾏良好，但是它们通常要求应⽤代码意识到消息丢失
的可能性。它们的容错程度极为有限：即使协议检测到并重传在⽹络中丢失的数据包，它们通常也只是
假设⽣产者和消费者始终在线。
* 如果消费者处于脱机状态，则可能会丢失其不可达时发送的消息。⼀些协议允许⽣产者重试失败的消息
传递，但当⽣产者崩溃时，它可能会丢失消息缓冲区及其本应发送的消息，这种⽅法可能就没⽤了。

#### 消息代理
* ⼀种⼴泛使⽤的替代⽅法是通过消息代理（message broker）（也称为消息队列（message
queue））发送消息，消息代理实质上是⼀种针对处理消息流⽽优化的数据库。它作为服务器运⾏，⽣
产者和消费者作为客户端连接到服务器。⽣产者将消息写⼊代理，消费者通过从代理那⾥读取来接收消
息。
* 通过将数据集中在代理上，这些系统可以更容易地容忍来来去去的客户端（连接，断开连接和崩溃），
⽽持久性问题则转移到代理的身上。⼀些消息代理只将消息保存在内存中，⽽另⼀些消息代理（取决于
配置）将其写⼊磁盘，以便在代理崩溃的情况下不会丢失。针对缓慢的消费者，它们通常会允许⽆上限
的排队（⽽不是丢弃消息或背压），尽管这种选择也可能取决于配置。
* 排队的结果是，消费者通常是异步（asynchronous）的：当⽣产者发送消息时，通常只会等待代理确
认消息已经被缓存，⽽不等待消息被消费者处理。向消费者递送消息将发⽣在未来某个未定的时间点
—— 通常在⼏分之⼀秒之内，但有时当消息堆积时会显著延迟。

#### 消息代理与数据库对⽐
* 有些消息代理甚⾄可以使⽤XA或JTA参与两阶段提交协议（参阅“实践中的分布式事务”）。这个功能与数
据库在本质上⾮常相似，尽管消息代理和数据库之间仍存在实践上很重要的差异。
  * 数据库通常保留数据直⾄显式删除，⽽⼤多数消息代理在消息成功递送给消费者时会⾃动删除消
息。这样的消息代理不适合⻓期的数据存储。
  * 由于它们很快就能删除消息，⼤多数消息代理都认为它们的⼯作集相当⼩—— 即队列很短。如果
代理需要缓冲很多消息，⽐如因为消费者速度较慢（如果内存装不下消息，可能会溢出到磁盘），
每个消息需要更⻓的处理时间，整体吞吐量可能会恶化。
  * 数据库通常⽀持⼆级索引和各种搜索数据的⽅式，⽽消息代理通常⽀持按照某种模式匹配主题，订
阅其⼦集。机制并不⼀样，对于客户端选择想要了解的数据的⼀部分，这是两种基本的⽅式。
  * 查询数据库时，结果通常基于某个时间点的数据快照；如果另⼀个客户端随后向数据库写⼊⼀些改
变了查询结果的内容，则第⼀个客户端不会发现其先前结果现已过期（除⾮它重复查询或轮询变
更）。相⽐之下，消息代理不⽀持任意查询，但是当数据发⽣变化时（即新消息可⽤时），它们会
通知客户端。
#### 多个消费者
* 负载均衡（load balance）
* 扇出（fan-out）
...
#### 确认与重新交付
* 消费随时可能会崩溃，所以有⼀种可能的情况是：代理向消费者递送消息，但消费者没有处理，或者在
消费者崩溃之前只进⾏了部分处理。为了确保消息不会丢失，消息代理使⽤确认
（acknowledgments）：客户端必须显式告知代理消息处理完毕的时间，以便代理能将消息从队列中
移除。
* 如果与客户端的连接关闭，或者代理超出⼀段时间未收到确认，代理则认为消息没有被处理，因此它将
消息再递送给另⼀个消费者。 （请注意可能发⽣这样的情况，消息实际上是处理完毕的，但确认在⽹络
中丢失了。需要⼀种原⼦提交协议才能处理这种情况，正如在“实践中的分布式事务”中所讨论的那样）。
* 当与负载均衡相结合时，这种重传⾏为对消息的顺序有种有趣的影响。在图11-2中，消费者通常按照⽣
产者发送的顺序处理消息。然⽽消费者2在处理消息m3时崩溃，与此同时消费者1正在处理消息m4。未
确认的消息m3随后被重新发送给消费者1，结果消费者1按照m4，m3，m5的顺序处理消息。因此m3
和m4的交付顺序与以⽣产者1的发送顺序不同。
* 即使消息代理试图保留消息的顺序（如JMS和AMQP标准所要求的），负载均衡与重传的组合也不可避
免地导致消息被重新排序。为避免此问题，你可以让每个消费者使⽤单独的队列（即不使⽤负载均衡功
能）。如果消息是完全独⽴的，则消息顺序重排并不是⼀个问题。但正如我们将在本章后续部分所述，
如果消息之间存在因果依赖关系，这就是⼀个很重要的问题。

### 分区⽇志
* 如果你将新的消费者添加到消息系统，通常只能接收到消费者注册之后开始发送的消息。先前的任何消
息都随⻛⽽逝，⼀去不复返。作为对⽐，你可以随时为⽂件和数据库添加新的客户端，且能读取任意久
远的数据（只要应⽤没有显式覆盖或删除这些数据）。
* 为什么我们不能把它俩杂交⼀下，既有数据库的持久存储⽅式，⼜有消息传递的低延迟通知？这就是基
于⽇志的消息代理（log-based message brokers）背后的想法。

#### 使⽤⽇志进⾏消息存储
* ⽇志只是磁盘上简单的仅追加记录序列。我们先前在第3章中⽇志结构存储引擎和预写式⽇志的上下⽂
中讨论了⽇志，在第5章复制的上下⽂⾥也讨论了它。
* 同样的结构可以⽤于实现消息代理：⽣产者通过将消息追加到⽇志末尾来发送消息，⽽消费者通过依次
读取⽇志来接收消息。如果消费者读到⽇志末尾，则会等待新消息追加的通知。 Unix⼯具 tail -f 能
监视⽂件被追加写⼊的数据，基本上就是这样⼯作的。
* 为了扩展到⽐单个磁盘所能提供的更⾼吞吐量，可以对⽇志进⾏分区（在第6章的意义上）。不同的分
区可以托管在不同的机器上，且每个分区都拆分出⼀份能独⽴于其他分区进⾏读写的⽇志。⼀个主题可
以定义为⼀组携带相同类型消息的分区。这种⽅法如图11-3所示。
* 在每个分区内，代理为每个消息分配⼀个单调递增的序列号或偏移量（offset）（在图11-3中，框中的
数字是消息偏移量）。这种序列号是有意义的，因为分区是仅追加写⼊的，所以分区内的消息是完全有
序的。没有跨不同分区的顺序保证。
* Apache Kafka，Amazon Kinesis Streams和Twitter的DistributedLog都
是基于⽇志的消息代理。 Google Cloud Pub/Sub在架构上类似，但对外暴露的是JMS⻛格的API，⽽不
是⽇志抽象。尽管这些消息代理将所有消息写⼊磁盘，但通过跨多台机器分区，每秒能够实现数
百万条消息的吞吐量，并通过复制消息来实现容错性。

#### ⽇志与传统消息相⽐
* 基于⽇志的⽅法天然⽀持扇出式消息传递，因为多个消费者可以独⽴读取⽇志，⽽不会相互影响 ——
读取消息不会将其从⽇志中删除。为了在⼀组消费者之间实现负载平衡，代理可以将整个分区分配给消
费者组中的节点，⽽不是将单条消息分配给消费者客户端。
* 每个客户端消费指派分区中的所有消息。然后使⽤分配的分区中的所有消息。通常情况下，当⼀个⽤户
被指派了⼀个⽇志分区时，它会以简单的单线程⽅式顺序地读取分区中的消息。这种粗粒度的负载均衡
⽅法有⼀些缺点
  * 共享消费主题⼯作的节点数，最多为该主题中的⽇志分区数，因为同⼀个分区内的所有消息被递送
到同⼀个节点.
  * 如果某条消息处理缓慢，则它会阻塞该分区中后续消息的处理（⼀种⾏⾸阻塞的形式；请参阅“描
述性能”）
* 因此在消息处理代价⾼昂，希望逐条并⾏处理，以及消息的顺序并没有那么重要的情况下，JMS/AMQP
⻛格的消息代理是可取的。另⼀⽅⾯，在消息吞吐量很⾼，处理迅速，顺序很重要的情况下，基于⽇志
的⽅法表现得⾮常好。

#### 消费者偏移量
* 顺序消费⼀个分区使得判断消息是否已经被处理变得相当容易：所有偏移量⼩于消费者的当前偏移量的
消息已经被处理，⽽具有更⼤偏移量的消息还没有被看到。因此，代理不需要跟踪确认每条消息，只需
要定期记录消费者的偏移即可。在这种⽅法减少了额外簿记开销，⽽且在批处理和流处理中采⽤这种⽅
法有助于提⾼基于⽇志的系统的吞吐量。
* 实际上，这种偏移量与单领导者数据库复制中常⻅的⽇志序列号⾮常相似，我们在“设置新从库”中讨论
了这种情况。在数据库复制中，⽇志序列号允许跟随者断开连接后，重新连接到领导者，并在不跳过任
何写⼊的情况下恢复复制。这⾥原理完全相同：消息代理的表现得像⼀个主库，⽽消费者就像⼀个从
库。
* 如果消费者节点失效，则失效消费者的分区将指派给其他节点，并从最后记录的偏移量开始消费消息。
如果消费者已经处理了后续的消息，但还没有记录它们的偏移量，那么重启后这些消息将被处理两次。
我们将在本章后⾯讨论这个问题的处理⽅法。

#### 磁盘空间使⽤
* 如果只追加写⼊⽇志，则磁盘空间终究会耗尽。为了回收磁盘空间，⽇志实际上被分割成段，并不时地
将旧段删除或移动到归档存储。
* 这就意味着如果⼀个慢消费者跟不上消息产⽣的速率⽽落后的太多，它的消费偏移量指向了删除的段，
那么它就会错过⼀些消息。实际上，⽇志实现了⼀个有限⼤⼩的缓冲区，当缓冲区填满时会丢弃旧消
息，它也被称为循环缓冲区（circular buffer）或环形缓冲区（ring buffer）。不过由于缓冲区在磁
盘上，因此可能相当的⼤。
* 让我们做个简单计算。在撰写本⽂时，典型的⼤型硬盘容量为6TB，顺序写⼊吞吐量为150MB/s。如果
以最快的速度写消息，则需要⼤约11个⼩时才能填满磁盘。因⽽磁盘可以缓冲11个⼩时的消息，之后它
将开始覆盖旧的消息。即使使⽤多个磁盘和机器，这个⽐率也是⼀样的。实践中的部署很少能⽤满磁盘
的写⼊带宽，所以通常可以保存⼀个⼏天甚⾄⼏周的⽇志缓冲区。
* 不管保留多⻓时间的消息，⽇志的吞吐量或多或少保持不变，因为⽆论如何，每个消息都会被写⼊磁盘
。这种⾏为与默认将消息保存在内存中，仅当队列太⻓时才写⼊磁盘的消息传递系统形成鲜明对
⽐。当队列很短时，这些系统⾮常快；⽽当这些系统开始写⼊磁盘时，就要慢的多，所以吞吐量取决于
保留的历史数量。

#### 当消费者跟不上⽣产者时
* 在“消息传递系统”中，如果消费者⽆法跟上⽣产者发送信息的速度时，我们讨论了三种选择：丢弃信
息，进⾏缓冲或施加背压。在这种分类法⾥，基于⽇志的⽅法是缓冲的⼀种形式，具有很⼤，但⼤⼩固
定的缓冲区（受可⽤磁盘空间的限制）。
* 如果消费者远远落后，⽽所要求的信息⽐保留在磁盘上的信息还要旧，那么它将不能读取这些信息，所
以代理实际上丢弃了⽐缓冲区容量更⼤的旧信息。你可以监控消费者落后⽇志头部的距离，如果落后太
多就发出报警。由于缓冲区很⼤，因⽽有⾜够的时间让⼈类运维来修复慢消费者，并在消息开始丢失之
前让其赶上。
* 即使消费者真的落后太多开始丢失消息，也只有那个消费者受到影响；它不会中断其他消费者的服务。
这是⼀个巨⼤的运维优势：你可以实验性地消费⽣产⽇志，以进⾏开发，测试或调试，⽽不必担⼼会中
断⽣产服务。当消费者关闭或崩溃时，会停⽌消耗资源，唯⼀剩下的只有消费者偏移量。
* 这种⾏为也与传统的信息代理形成了鲜明对⽐，在那种情况下，你需要⼩⼼地删除那些消费者已经关闭
的队列—— 否则那些队列就会累积不必要的消息，从其他仍活跃的消费者那⾥占⾛内存。

#### 重播旧信息
* 我们之前提到，使⽤AMQP和JMS⻛格的消息代理，处理和确认消息是⼀个破坏性的操作，因为它会导
致消息在代理上被删除。另⼀⽅⾯，在基于⽇志的消息代理中，使⽤消息更像是从⽂件中读取数据：这
是只读操作，不会更改⽇志。
* 除了消费者的任何输出之外，处理的唯⼀副作⽤是消费者偏移量的前进。但偏移量是在消费者的控制之
下的，所以如果需要的话可以很容易地操纵：例如你可以⽤昨天的偏移量跑⼀个消费者副本，并将输出
写到不同的位置，以便重新处理最近⼀天的消息。你可以使⽤各种不同的处理代码重复任意次。
* 这⼀⽅⾯使得基于⽇志的消息传递更像上⼀章的批处理，其中衍⽣数据通过可重复的转换过程与输⼊数
据显式分离。它允许进⾏更多的实验，更容易从错误和漏洞中恢复，使其成为在组织内集成数据流的良
好⼯具。

### 流与数据库
* 我们已经在消息代理和数据库之间进⾏了⼀些⽐较。尽管传统上它们被视为单独的⼯具类别，但是我们
看到基于⽇志的消息代理已经成功地从数据库中获取灵感并将其应⽤于消息传递。我们也可以反过来：
从消息传递和流中获取灵感，并将它们应⽤于数据库。
* 我们之前曾经说过，事件是某个时刻发⽣的事情的记录。发⽣的事情可能是⽤户操作（例如键⼊搜索查
询）或读取传感器，但也可能是写⼊数据库。某些东⻄被写⼊数据库的事实是可以被捕获，存储和处理
的事件。这⼀观察结果表明，数据库和数据流之间的联系不仅仅是磁盘⽇志的物理存储 —— ⽽是更深
层的联系。
* 事实上，复制⽇志（参阅“复制⽇志的实现”）是数据库写⼊事件的流，由主库在处理事务时⽣成。从库
将写⼊流应⽤到它们⾃⼰的数据库副本，从⽽最终得到相同数据的精确副本。复制⽇志中的事件描述发
⽣的数据更改。
* 我们还在“全序⼴播”中遇到了状态机复制原理，其中指出：如果每个事件代表对数据库的写⼊，并且每
个副本按相同的顺序处理相同的事件，则副本将达到相同的最终状态 （假设处理⼀个事件是⼀个确定性
的操作）。这是事件流的⼜⼀种场景！

### 保持系统同步
* 正如我们在本书中所看到的，没有⼀个系统能够满⾜所有的数据存储，查询和处理需求。在实践中，⼤
多数重要应⽤都需要组合使⽤⼏种不同的技术来满⾜所有的需求：例如，使⽤OLTP数据库来为⽤户请
求提供服务，使⽤缓存来加速常⻅请求，使⽤全⽂索引搜索处理搜索查询，使⽤数据仓库⽤于分析。每
⼀个组件都有⾃⼰的数据副本，以⾃⼰的表示存储，并根据⾃⼰的⽬的进⾏优化。
* 由于相同或相关的数据出现在了不同的地⽅，因此相互间需要保持同步：如果某个项⽬在数据库中被更
新，它也应当在缓存，搜索索引和数据仓库中被更新。对于数据仓库，这种同步通常由ETL进程执⾏
（参⻅“数据仓库”），通常是先取得数据库的完整副本，然后执⾏转换，并批量加载到数据仓库中 ——
换句话说，批处理。我们在“批量⼯作流的输出”中同样看到了如何使⽤批处理创建搜索索引，推荐系统
和其他衍⽣数据系统。
* 如果周期性的完整数据库转储过于缓慢，有时会使⽤的替代⽅法是双写（dual write），其中应⽤代码
在数据变更时明确写⼊每个系统：例如，⾸先写⼊数据库，然后更新搜索索引，然后使缓存项失效（甚
⾄同时执⾏这些写⼊）。
* 但是，双写有⼀些严重的问题，其中⼀个是竞争条件，如图11-4所示。在这个例⼦中，两个客户端同时
想要更新⼀个项⽬X：客户端1想要将值设置为A，客户端2想要将其设置为B。两个客户端⾸先将新值写
⼊数据库，然后将其写⼊到搜索索引。因为运⽓不好，这些请求的时序是交错的：数据库⾸先看到来⾃
客户端1的写⼊将值设置为A，然后来⾃客户端2的写⼊将值设置为B，因此数据库中的最终值为B。搜索
索引⾸先看到来⾃客户端2的写⼊，然后是客户端1的写⼊，所以搜索索引中的最终值是A。即使没发⽣
错误，这两个系统现在也永久地不⼀致了。
* 除⾮有⼀些额外的并发检测机制，例如我们在“检测并发写⼊”中讨论的版本向量，否则你甚⾄不会意识
到发⽣了并发写⼊ —— ⼀个值将简单地以⽆提示⽅式覆盖另⼀个值。
* 双重写⼊的另⼀个问题是，其中⼀个写⼊可能会失败，⽽另⼀个成功。这是⼀个容错问题，⽽不是⼀个
并发问题，但也会造成两个系统互相不⼀致的结果。确保它们要么都成功要么都失败，是原⼦提交问题
的⼀个例⼦，解决这个问题的代价是昂贵的。
* 如果你只有⼀个单领导者复制的数据库，那么这个领导者决定了写⼊顺序，⽽状态机复制⽅法可以在数
据库副本上⼯作。然⽽，在图11-4中，没有单个主库：数据库可能有⼀个领导者，搜索索引也可能有⼀
个领导者，但是两者都不追随对⽅，所以可能会发⽣冲突（参⻅“多领导者复制“）。
### 变更数据捕获
* ⼤多数数据库的复制⽇志的问题在于，它们⼀直被当做数据库的内部实现细节，⽽不是公开的API。客
户端应该通过其数据模型和查询语⾔来查询数据库，⽽不是解析复制⽇志并尝试从中提取数据。
* 数⼗年来，许多数据库根本没有记录在档的，获取变更⽇志的⽅式。由于这个原因，捕获数据库中所有
的变更，然后将其复制到其他存储技术（搜索索引，缓存，数据仓库）中是相当困难的。
* 最近，⼈们对变更数据捕获（change data capture, CDC）越来越感兴趣，这是⼀种观察写⼊数据库
的所有数据变更，并将其提取并转换为可以复制到其他系统中的形式的过程。 CDC是⾮常有意思的，尤
其是当变更能在被写⼊后⽴刻⽤于流时。
* 例如，你可以捕获数据库中的变更，并不断将相同的变更应⽤⾄搜索索引。如果变更⽇志以相同的顺序
应⽤，则可以预期搜索索引中的数据与数据库中的数据是匹配的。搜索索引和任何其他衍⽣数据系统只
是变更流的消费者，如图11-5所示。

#### 变更数据捕获的实现
* 我们可以将⽇志消费者叫做衍⽣数据系统，正如在第三部分的介绍中所讨论的：存储在搜索索引和数据
仓库中的数据，只是记录系统数据的额外视图。变更数据捕获是⼀种机制，可确保对记录系统所做的所
有更改都反映在派⽣数据系统中，以便派⽣系统具有数据的准确副本。
* 从本质上说，变更数据捕获使得⼀个数据库成为领导者（被捕获变化的数据库），并将其他组件变为追
随者。基于⽇志的消息代理⾮常适合从源数据库传输变更事件，因为它保留了消息的顺序（避免了图
11-2的重新排序问题）。
* 数据库触发器可⽤来实现变更数据捕获（参阅“基于触发器的复制”），通过注册观察所有变更的触发
器，并将相应的变更项写⼊变更⽇志表中。但是它们往往是脆弱的，⽽且有显著的性能开销。解析复制
⽇志可能是⼀种更稳健的⽅法，但它也很有挑战，例如应对模式变更。

#### 初始快照
* 如果你拥有所有对数据库进⾏变更的⽇志，则可以通过重放该⽇志，来重建数据库的完整状态。但是在
许多情况下，永远保留所有更改会耗费太多磁盘空间，且重放过于费时，因此⽇志需要被截断。
* 例如，构建新的全⽂索引需要整个数据库的完整副本 —— 仅仅应⽤最近变更的⽇志是不够的，因为这
样会丢失最近未曾更新的项⽬。因此，如果你没有完整的历史⽇志，则需要从⼀个⼀致的快照开始，如
先前上的“设置新的从库”中所述。
* 数据库的快照必须与变更⽇志中的已知位置或偏移量相对应，以便在处理完快照后知道从哪⾥开始应⽤
变更。⼀些CDC⼯具集成了这种快照功能，⽽其他⼯具则把它留给你⼿动执⾏。

#### ⽇志压缩
* 如果你只能保留有限的历史⽇志，则每次要添加新的衍⽣数据系统时，都需要做⼀次快照。但⽇志压缩
（log compaction）提供了⼀个很好的备选⽅案。
* 我们之前在⽇志结构存储引擎的上下⽂中讨论了“Hash索引”中的⽇志压缩（参⻅图3-2的示例）。原理
很简单：存储引擎定期在⽇志中查找具有相同键的记录，丢掉所有重复的内容，并只保留每个键的最新
更新。这个压缩与合并过程在后台运⾏。
* 在⽇志结构存储引擎中，具有特殊值NULL（墓碑（tombstone））的更新表示该键被删除，并会在⽇
志压缩过程中被移除。但只要键不被覆盖或删除，它就会永远留在⽇志中。这种压缩⽇志所需的磁盘空
间仅取决于数据库的当前内容，⽽不取决于数据库中曾经发⽣的写⼊次数。如果相同的键经常被覆盖写
⼊，则先前的值将最终将被垃圾回收，只有最新的值会保留下来。
* 在基于⽇志的消息代理与变更数据捕获的上下⽂中也适⽤相同的想法。如果CDC系统被配置为，每个变
更都包含⼀个主键，且每个键的更新都替换了该键以前的值，那么只需要保留对键的最新写⼊就⾜够
了。
* 现在，⽆论何时需要重建衍⽣数据系统（如搜索索引），你可以从压缩⽇志主题0偏移量处启动新的消
费者，然后依次扫描⽇志中的所有消息。⽇志能保证包含数据库中每个键的最新值（也可能是⼀些较旧
的值）—— 换句话说，你可以使⽤它来获取数据库内容的完整副本，⽽⽆需从CDC源数据库取⼀个快
照。
* Apache Kafka⽀持这种⽇志压缩功能。正如我们将在本章后⾯看到的，它允许消息代理被当成持久性存
储使⽤，⽽不仅仅是⽤于临时消息。

#### 变更流的API⽀持
* 越来越多的数据库开始将变更流作为第⼀类的接⼝，⽽不像传统上要去做加装改造，费⼯夫逆向⼯程⼀
个CDC。例如，RethinkDB允许查询订阅通知，当查询结果变更时获得通知【36】，Firebase 【37】和
CouchDB 【38】基于变更流进⾏同步，该变更流同样可⽤于应⽤。⽽Meteor使⽤MongoDB oplog订
阅数据变更，并改变了⽤户接⼝。
* VoltDB允许事务以流的形式连续地从数据库中导出数据【40】。数据库将关系数据模型中的输出流表示
为⼀个表，事务可以向其中插⼊元组，但不能查询。已提交事务按照提交顺序写⼊这个特殊表，⽽流则
由该表中的元组⽇志构成。外部消费者可以异步消费该⽇志，并使⽤它来更新衍⽣数据系统。
* Kafka Connect 【41】致⼒于将⼴泛的数据库系统的变更数据捕获⼯具与Kafka集成。⼀旦变更事件进
⼊Kafka中，它就可以⽤于更新衍⽣数据系统，⽐如搜索索引，也可以⽤于本章稍后讨论的流处理系
统。

### 事件溯源
* 我们在这⾥讨论的想法和事件溯源（ Event Sourcing）之间有⼀些相似之处，这是⼀个在领域驱动设
计（domain-driven design, DDD）社区中折腾出来的技术。我们将简要讨论事件溯源，因为它包含
了⼀些关于流处理系统的有⽤想法。
* 与变更数据捕获类似，事件溯源涉及到将所有对应⽤状态的变更存储为变更事件⽇志。最⼤的区别是事
件溯源将这⼀想法应⽤到了⼏个不同的抽象层次上：
  * 在变更数据捕获中，应⽤以可变⽅式（mutable way）使⽤数据库，任意更新和删除记录。变更
⽇志是从数据库的底层提取的（例如，通过解析复制⽇志），从⽽确保从数据库中提取的写⼊顺序
与实际写⼊的顺序相匹配，从⽽避免图11-4中的竞态条件。写⼊数据库的应⽤不需要知道CDC的存
在。
  * 在事件溯源中，应⽤逻辑显式构建在写⼊事件⽇志的不可变事件之上。在这种情况下，事件存储是
仅追加写⼊的，更新与删除是不⿎励的或禁⽌的。事件被设计为旨在反映应⽤层⾯发⽣的事情，⽽
不是底层的状态变更。
* 事件源是⼀种强⼤的数据建模技术：从应⽤的⻆度来看，将⽤户的⾏为记录为不可变的事件更有意义，
⽽不是在可变数据库中记录这些⾏为的影响。事件代理使得应⽤随时间演化更为容易，通过事实更容易
理解事情发⽣的原因，使得调试更为容易，并有利于防⽌应⽤Bug。
* 例如，存储“学⽣取消选课”事件以中性的⽅式清楚地表达了单个⾏为的意图，⽽副作⽤“从注册表中删除
了⼀个条⽬，⽽⼀条取消原因被添加到学⽣反馈表“则嵌⼊了很多有关稍后数据使⽤⽅式的假设。如果引
⼊⼀个新的应⽤功能，例如“将位置留给等待列表中的下⼀个⼈” —— 事件溯源⽅法允许将新的副作⽤轻
松地链接⾄现有事件之后。
* 事件溯源类似于编年史（chronicle）数据模型，事件⽇志与星型模式中的事实表之间也存在相
似之处（参阅“星型和雪花型：分析的模式”） 。
* 诸如Event Store这样的专业数据库已经被开发出来，供使⽤事件溯源的应⽤使⽤，但总的来
说，这种⽅法独⽴于任何特定的⼯具。传统的数据库或基于⽇志的消息代理也可以⽤来构建这种⻛格的
应⽤。
#### 从事件⽇志中派⽣出当前状态
#### 命令和事件
* 事件溯源的哲学是仔细区分事件（event）和命令（command）。当来⾃⽤户的请求刚到达
时，它⼀开始是⼀个命令：在这个时间点上它仍然可能可能失败，⽐如，因为违反了⼀些完整性条件。
应⽤必须⾸先验证它是否可以执⾏该命令。如果验证成功并且命令被接受，则它变为⼀个持久化且不可
变的事件。
* 例如，如果⽤户试图注册特定⽤户名，或预定⻜机或剧院的座位，则应⽤需要检查⽤户名或座位是否已
被占⽤。 （先前在“容错概念”中讨论过这个例⼦）当检查成功时，应⽤可以⽣成⼀个事件，指示特定的
⽤户名是由特定的⽤户ID注册的，座位已经预留给特定的顾客。
* 在事件⽣成的时刻，它就成为了事实（fact）。即使客户稍后决定更改或取消预订，他们之前曾预定了
某个特定座位的事实仍然成⽴，⽽更改或取消是之后添加的单独的事件。
* 事件流的消费者不允许拒绝事件：当消费者看到事件时，它已经成为⽇志中不可变的⼀部分，并且可能
已经被其他消费者看到了。因此任何对命令的验证，都需要在它成为事件之前同步完成。例如，通过使
⽤⼀个可⾃动验证命令的可序列化事务来发布事件。
* 或者，预订座位的⽤户请求可以拆分为两个事件：第⼀个是暂时预约，第⼆个是验证预约后的独⽴的确
认事件（如“使⽤全序⼴播实现线性⼀致存储”中所述） 。这种分割⽅式允许验证发⽣在⼀个异步的过程
中。

#### 状态，流和不变性
* 我们在第10章中看到，批处理因其输⼊⽂件不变性⽽受益良多，你可以在现有输⼊⽂件上运⾏实验性处
理作业，⽽不⽤担⼼损坏它们。这种不变性原则也是使得事件溯源与变更数据捕获如此强⼤的原因。
* 我们通常将数据库视为应⽤程序当前状态的存储 —— 这种表示针对读取进⾏了优化，⽽且通常对于服
务查询⽽⾔是最为⽅便的表示。状态的本质是，它会变化，所以数据库才会⽀持数据的增删改。这⼜是
如何符合不变性的呢？
* 只要你的状态发⽣了变化，那么这个状态就是这段时间中事件修改的结果。例如，当前可⽤的座位列表
是已处理预订产⽣的结果，当前帐户余额是帐户中的借与贷的结果，⽽Web服务器的响应时间图，是所
有已发⽣Web请求的独⽴响应时间的聚合结果。
* ⽆论状态如何变化，总是有⼀系列事件导致了这些变化。即使事情已经执⾏与回滚，这些事件出现是始
终成⽴的。关键的想法是：可变的状态与不可变事件的仅追加⽇志相互之间并不⽭盾：它们是⼀体两
⾯，互为阴阳的。所有变化的⽇志—— 变化⽇志（change log），表示了随时间演变的状态。

#### 不可变事件的优点
* 数据库中的不变性是⼀个古⽼的概念。例如，会计在⼏个世纪以来⼀直在财务记账中应⽤不变性。⼀笔
交易发⽣时，它被记录在⼀个仅追加写⼊的分类帐中，实质上是描述货币，商品或服务转⼿的事件⽇
志。账⽬，⽐如利润、亏损、资产负债表，是从分类账中的交易求和衍⽣⽽来。
* 如果发⽣错误，会计师不会删除或更改分类帐中的错误交易 —— ⽽是添加另⼀笔交易以补偿错误，例
如退还⼀⽐不正确的费⽤。不正确的交易将永远保留在分类帐中，对于审计⽽⾔可能⾮常重要。如果从
不正确的分类账衍⽣出的错误数字已经公布，那么下⼀个会计周期的数字就会包括⼀个更正。这个过程
在会计事务中是很常⻅的。
* 尽管这种可审计性在⾦融系统中尤其重要，但对于不受这种严格监管的许多其他系统，也是很有帮助
的。如“批处理输出的哲学”中所讨论的，如果你意外地部署了将错误数据写⼊数据库的错误代码，当代
码会破坏性地覆写数据时，恢复要困难得多。使⽤不可变事件的仅追加⽇志，诊断问题与故障恢复就要
容易的多。
* 不可变的事件也包含了⽐当前状态更多的信息。例如在购物⽹站上，顾客可以将物品添加到他们的购物
⻋，然后再将其移除。虽然从履⾏订单的⻆度，第⼆个事件取消了第⼀个事件，但对分析⽬的⽽⾔，知
道客户考虑过某个特定项⽽之后⼜反悔，可能是很有⽤的。也许他们会选择在未来购买，或者他们已经
找到了替代品。这个信息被记录在事件⽇志中，但对于移出购物⻋就删除记录的数据库⽽⾔，这个信息
在移出购物⻋时可能就丢失。

#### 从同⼀事件⽇志中派⽣多个视图
* 此外，通过从不变的事件⽇志中分离出可变的状态，你可以针对不同的读取⽅式，从相同的事件⽇志中
衍⽣出⼏种不同的表现形式。效果就像⼀个流的多个消费者⼀样（图11-5）：例如，分析型数据库
Druid使⽤这种⽅式直接从Kafka摄取数据，Pistachio是⼀个分布式的键值存储，使⽤Kafka作为
提交⽇志，Kafka Connect能将来⾃Kafka的数据导出到各种不同的数据库与索引。这对
于许多其他存储和索引系统（如搜索服务器）来说是很有意义的，当系统要从分布式⽇志中获取输⼊时
亦然（参阅“保持系统同步”）
* 添加从事件⽇志到数据库的显式转换，能够使应⽤更容易地随时间演进：如果你想要引⼊⼀个新功能，
以新的⽅式表示现有数据，则可以使⽤事件⽇志来构建⼀个单独的，针对新功能的读取优化视图，⽆需
修改现有系统⽽与之共存。并⾏运⾏新旧系统通常⽐在现有系统中执⾏复杂的模式迁移更容易。⼀旦不
再需要旧的系统，你可以简单地关闭它并回收其资源。
* 如果你不需要担⼼如何查询与访问数据，那么存储数据通常是⾮常简单的。模式设计，索引和存储引擎
的许多复杂性，都是希望⽀持某些特定查询和访问模式的结果（参⻅第3章）。出于这个原因，通过将
数据写⼊的形式与读取形式相分离，并允许⼏个不同的读取视图，你能获得很⼤的灵活性。这个想法有
时被称为命令查询责任分离（command query responsibility segregation, CQRS）。
* 数据库和模式设计的传统⽅法是基于这样⼀种谬论，数据必须以与查询相同的形式写⼊。如果可以将数
据从针对写⼊优化的事件⽇志转换为针对读取优化的应⽤状态，那么有关规范化和⾮规范化的争论就变
得⽆关紧要了（参阅“多对⼀和多对多的关系”）：在针对读取优化的视图中对数据进⾏⾮规范化是完全
合理的，因为翻译过程提供了使其与事件⽇志保持⼀致的机制。
* 在“描述负载”中，我们讨论了推特主⻚时间线，它是特定⽤户关注⼈群所发推特的缓存（类似邮箱）。
这是针对读取优化的状态的⼜⼀个例⼦：主⻚时间线是⾼度⾮规范化的，因为你的推⽂与所有粉丝的时
间线都构成了重复。然⽽，扇出服务保持了这种重复状态与新推特以及新关注关系的同步，从⽽保证了
重复的可管理性。

#### 并发控制
* 事件溯源和变更数据捕获的最⼤缺点是，事件⽇志的消费者通常是异步的，所以可能会出现这样的情
况：⽤户会写⼊⽇志，然后从⽇志衍⽣视图中读取，结果发现他的写⼊还没有反映在读取视图中。我们
之前在在“读⼰之写”中讨论了这个问题以及可能的解决⽅案。
* ⼀种解决⽅案是将事件附加到⽇志时同步执⾏读取视图的更新。⽽将这些写⼊操作合并为⼀个原⼦单元
需要事务，所以要么将事件⽇志和读取视图保存在同⼀个存储系统中，要么就需要跨不同系统进⾏分布
式事务。或者，你也可以使⽤在“使⽤全序⼴播实现线性化存储”中讨论的⽅法。
* 另⼀⽅⾯，从事件⽇志导出当前状态也简化了并发控制的某些部分。许多对于多对象事务的需求（参
阅“单对象和多对象操作”）源于单个⽤户操作需要在多个不同的位置更改数据。通过事件溯源，你可以
设计⼀个⾃包含的事件以表示⼀个⽤户操作。然后⽤户操作就只需要在⼀个地⽅进⾏单次写⼊操作 ——
即将事件附加到⽇志中 —— 这个还是很容易使原⼦化的。
* 如果事件⽇志与应⽤状态以相同的⽅式分区（例如，处理分区3中的客户事件只需要更新分区3中的应⽤
状态），那么直接使⽤单线程⽇志消费者就不需要写⼊并发控制了。它从设计上⼀次只处理⼀个事件
（参阅“真的的串⾏执⾏”）。⽇志通过在分区中定义事件的序列顺序，消除了并发性的不确定性
【24】。如果⼀个事件触及多个状态分区，那么需要做更多的⼯作，我们将在第12章讨论。

#### 不变性的限制
* 许多不使⽤事件溯源模型的系统也还是依赖不可变性：各种数据库在内部使⽤不可变的数据结构或多版
本数据来⽀持时间点快照（参⻅“索引和快照隔离” ）。 Git，Mercurial和Fossil等版本控制系统也依靠
不可变的数据来保存⽂件的版本历史记录。
* 永远保持所有变更的不变历史，在多⼤程度上是可⾏的？答案取决于数据集的流失率。⼀些⼯作负载主
要是添加数据，很少更新或删除；它们很容易保持不变。其他⼯作负载在相对较⼩的数据集上有较⾼的
更新/删除率；在这些情况下，不可变的历史可能增⾄难以接受的巨⼤，碎⽚化可能成为⼀个问题，压缩
与垃圾收集的表现对于运维的稳健性变得⾄关重要。
* 除了性能⽅⾯的原因外，也可能有出于管理⽅⾯的原因需要删除数据的情况，尽管这些数据都是不可变
的。例如，隐私条例可能要求在⽤户关闭帐户后删除他们的个⼈信息，数据保护⽴法可能要求删除错误
的信息，或者可能需要阻⽌敏感信息的意外泄露。
* 在这种情况下，仅仅在⽇志中添加另⼀个事件来指明先前的数据应该被视为删除是不够的 —— 你实际
上是想改写历史，并假装数据从⼀开始就没有写⼊。例如，Datomic管这个特性叫切除（excision），⽽Fossil版本控制系统有⼀个类似的概念叫避免（shunning）。
* 真正删除数据是⾮常⾮常困难的【64】，因为副本可能存在于很多地⽅：例如，存储引擎，⽂件系统和
SSD通常会向⼀个新位置写⼊，⽽不是原地覆盖旧数据，⽽备份通常是特意做成不可变的，防⽌
意外删除或损坏。删除更多的是“使取回数据更困难”，⽽不是“使取回数据不可能”。⽆论如何，有时你
必须得尝试，正如我们在“⽴法与⾃律”中所看到的。

### 流处理
* 你可以将事件中的数据写⼊数据库，缓存，搜索索引或类似的存储系统，然后能被其他客户端查
询。如图11-5所示，这是数据库与系统其他部分发⽣变更保持同步的好⽅法 —— 特别是当流消费
者是写⼊数据库的唯⼀客户端时。如“批处理⼯作流的输出”中所讨论的，它是写⼊存储系统的流等
价物。
* 你能以某种⽅式将事件推送给⽤户，例如发送报警邮件或推送通知，或将事件流式传输到可实时显
示的仪表板上。在这种情况下，⼈是流的最终消费者。
* 你可以处理⼀个或多个输⼊流，并产⽣⼀个或多个输出流。流可能会经过由⼏个这样的处理阶段组
成的流⽔线，最后再输出（选项1或2）。
* 在本章的剩余部分中，我们将讨论选项3：处理流以产⽣其他衍⽣流。处理这样的流的代码⽚段，被称
为算⼦（operator）或作业（job）。它与我们在第10章中讨论过的Unix进程和MapReduce作业密切
相关，数据流的模式是相似的：⼀个流处理器以只读的⽅式使⽤输⼊流，并将其输出以仅追加的⽅式写
⼊⼀个不同的位置。
* 与批量作业相⽐的⼀个关键区别是，流不会结束。这种差异会带来很多隐含的结果。正如本章开始部分
所讨论的，排序对⽆界数据集没有意义，因此⽆法使⽤排序合并联接（请参阅“Reduce端连接与分
组”）。容错机制也必须改变：对于已经运⾏了⼏分钟的批处理作业，可以简单地从头开始重启失败任
务，但是对于已经运⾏数年的流作业，重启后从头开始跑可能并不是⼀个可⾏的选项。

#### 流处理的应⽤
* ⻓期以来，流处理⼀直⽤于监控⽬的，如果某个事件发⽣，单位希望能得到警报。例如：
  * 欺诈检测系统需要确定信⽤卡的使⽤模式是否有意外地变化，如果信⽤卡可能已被盗刷，则锁卡。
  * 交易系统需要检查⾦融市场的价格变化，并根据指定的规则进⾏交易。
  * 制造系统需要监控⼯⼚中机器的状态，如果出现故障，可以快速定位问题。
  * 军事和情报系统需要跟踪潜在侵略者的活动，并在出现袭击征兆时发出警报。
* 这些类型的应⽤需要⾮常精密复杂的模式匹配与相关检测。然⽽随着时代的进步，流处理的其他⽤途也
开始出现。在本节中，我们将简要⽐较⼀下这些应⽤。
  * 复合事件处理（complex, event processing, CEP）是20世纪90年代为分析事件流⽽开发出的⼀种⽅
法，尤其适⽤于需要搜索某些事件模式的应⽤。与正则表达式允许你在字符串中搜索特定字
符模式的⽅式类似，CEP允许你指定规则以在流中搜索某些事件模式。
  * CEP系统通常使⽤⾼层次的声明式查询语⾔，⽐如SQL，或者图形⽤户界⾯，来描述应该检测到的事件
模式。这些查询被提交给处理引擎，该引擎消费输⼊流，并在内部维护⼀个执⾏所需匹配的状态机。当
发现匹配时，引擎发出⼀个复合事件（complex event）（因此得名），并附有检测到的事件模式详情
  * 在这些系统中，查询和数据之间的关系与普通数据库相⽐是颠倒的。通常情况下，数据库会持久存储数
据，并将查询视为临时的：当查询进⼊时，数据库搜索与查询匹配的数据，然后在查询完成时丢掉查
询。 CEP引擎反转了⻆⾊：查询是⻓期存储的，来⾃输⼊流的事件不断流过它们，搜索匹配事件模式的
查询
#### 流分析
* 测量某种类型事件的速率（每个时间间隔内发⽣的频率)。
* 滚动计算⼀段时间窗⼝内某个值的平均值。
* 将当前的统计值与先前的时间区间的值对⽐（例如，检测趋势，当指标与上周同⽐异常偏⾼或偏低
时报警）。
