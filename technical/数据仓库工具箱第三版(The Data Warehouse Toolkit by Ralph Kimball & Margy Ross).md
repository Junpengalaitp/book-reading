...
* 获取是将数据从操作型系统导入到数据仓库这一ETL过程的第一步，意味着读取源数据并将需要的数据复制到ETL系统中以利于后续的处理操作。
...
### 6.2.1 事实表规范化
* 保留单一的，通用的事实数量以及维度，用于区分度量类型。
* 在事实集合非常巨大时，该技术具有意义。
* 应该慎重使用规范化事实表，因为会导致行数大量增加

### 6.2.2 维度角色扮演
* 同一维度在单一事实表中扮演不同角色，例如下单日期和发货日期是同一天。
* 角色扮演与总线矩阵：在矩阵的一个单元中标明多种角色。
...
* 产品维度表的共同特征
  ...
  * 一个或多个属性层次，加上没有层次的属性
  * 重新建立操作型产品代码到代理键的映射
* 精通操作型人员的存在有助于建立和维护产品维度，但必须开展大量的转换和管理步骤以将操作型文件转换成维度表
  * 增加描述性属性值以扩大或替换操作型代码
...
#### 1. 单一维度表与多维度表
* 一对一关系或多对多关系
* 销售代理和客户维度关系会随着时间发生变化
* 客户维度非常巨大吗
* 销售代理和客户维度独立地参于其他事实表吗
* 业务认为销售代理和客户应该是不同的事情吗

#### 2. 应用于客户/代理分配的无事实的事实表
...
### 6.2.9 多币种
### 6.2.10 不同粒度的事务事实
### 6.2.11 另外一种需要避免的表头/明细模式

## 6.3 发票事务
### 6.3.1 作为事实、维度或者两者兼顾的服务级性能
### 6.3.2 利润与损益事实
* 发货数量
* 扩展总金额
* 扩展津贴额
